...
start_date = '07-01 00:01:00'
end_date = '08-31 23:59:59'
...
# Input data set
input_data_set_summer = pd.DataFrame({'To': To, 'Ti_sp': Ti_sp,
                               'Φo': Φo, 'Φi': Φi, 'Qa': Qa, 'Φa': Φa,
                               'Etot': Etot})
input_data_set_summer.to_csv('input_data_set_summer.csv')
...
# INPUT DATA SET
# ==============
input_data_set = pd.read_csv('input_data_set_summer.csv',#on doit changer le fichier en fonction de ce que l'on veut montrer
                             index_col=0,
                             parse_dates=True)
input_data_set
...
fig_true, axs = plt.subplots(2, 1)
# plot outdoor and indoor temperature
axs[0].plot(t / 3600 / 24, data['To'], label='$θ_{outdoor}$')
axs[0].plot(t / 3600 / 24, y.values, label='$θ_{indoor}$')
axs[0].set(ylabel='Temperatures, $θ$ / [°C]',
           title='Simulation for summer weather with auxiliary sources with controller ')
...
fig_true.savefig("Summer_simulation_true.svg",dpi=300)
...
fig_false, axs = plt.subplots(2, 1)
# plot outdoor and indoor temperature
axs[0].plot(t / 3600 / 24, data['To'], label='$θ_{outdoor}$')
axs[0].plot(t / 3600 / 24, y.values, label='$θ_{indoor}$')
axs[0].set(ylabel='Temperatures, $θ$ / [°C]',
           title='Simulation for summer weather with auxiliary sources without controller')
axs[0].legend(loc='upper right')
...
fig_false.savefig("Summer_simulation_false.svg",dpi=300)