...
start_date = '01-01 00:01:00'
end_date = '02-28 23:59:59'
...
# Input data set
input_data_set_winter = pd.DataFrame({'To': To, 'Ti_sp': Ti_sp,
                               'Φo': Φo, 'Φi': Φi, 'Qa': Qa, 'Φa': Φa,
                               'Etot': Etot})
input_data_set_winter.to_csv('input_data_set_winter.csv')
...
# INPUT DATA SET
# ==============
input_data_set = pd.read_csv('input_data_set_winter.csv',#on doit changer le fichier en fonction de ce que l'on veut montrer
                             index_col=0,
                             parse_dates=True)
input_data_set
...
figtrue, axs = plt.subplots(2, 1)
# plot outdoor and indoor temperature
axs[0].plot(t / 3600 / 24, data['To'], label='$θ_{outdoor}$')
axs[0].plot(t / 3600 / 24, y.values, label='$θ_{indoor}$')
axs[0].set(ylabel='Temperatures, $θ$ / [°C]',
           title='Simulation for winter weather with auxiliary sources and controller')
...
figtrue.savefig("Winter_simulation_auxiliaire_tcontroller.svg",dpi=300)
...
figfalse, axs = plt.subplots(2, 1)
# plot outdoor and indoor temperature
axs[0].plot(t / 3600 / 24, data['To'], label='$θ_{outdoor}$')
axs[0].plot(t / 3600 / 24, y.values, label='$θ_{indoor}$')
axs[0].set(ylabel='Temperatures, $θ$ / [°C]',
           title='Simulation for winter weather with auxiliary sources without controller')
axs[0].legend(loc='upper right')
...
figfalse.tight_layout()
figfalse.savefig("Winter_simulation_auxiliaire_fcontroller.svg",dpi=300)